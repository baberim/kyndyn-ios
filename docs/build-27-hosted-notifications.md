...
The initial Worker exposes only a content-free health check. Notification APIs
fail closed until the following are implemented and reviewed:

1. authenticated, household-scoped device enrollment;
2. token replacement, revocation, and expiration;
3. D1 migrations with no names, quest titles, PIN material, CloudKit tokens,
   calendar details, precise location, or message content in diagnostic logs;
