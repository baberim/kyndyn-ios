...
@Observable final class CloudSyncController {
    ...
    private func notifyNewBroadcasts(
        ...
        // Hosted APNs already presents this announcement. Only schedule the
        // CloudKit-driven local alert as a fallback on unpaired devices.
        let needsLocalFallback = !HostedNotificationCoordinator.shared.isConnected
        ...
        for record in records where record.type == .familyBroadcast &&
            ...
            if needsLocalFallback && localMetadata?.lastMutationID != record.mutationID {
                do {
